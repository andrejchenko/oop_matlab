function selectXPixelPerClass_16BitVal_libSVMNormalization(obj,nObj)
...
for i =1: obj.numClasses
    ...
    obj.trainMatrix{i} = obj.libSVM_normalization(obj.trainMatrix{i});
    % Normalisation of the test data:
    %obj.testMatrix{i} = obj.normalizeX(obj.testMatrix{i});
    obj.testMatrix{i} = obj.libSVM_normalization(obj.testMatrix{i});
    ...
    if(obj.numNeigh > 0) 
        ...
        nObj.neighboursData{i} = obj.libSVM_normalization(nObj.neighboursData{i});
    end
end
obj.testLabels = testLabels;
