function stackingSvm_Sunsal_96F_0NN_0_9lambda_pines
    avgUnmixing = 0;
    iter = 100;
    for a = 1:iter
        ...
        cObj = CombineClass;
        cObj.combineMethodsSVMSunalNoValidation96F(obj,nObj,svmObj,sunObj);

        cObj.trainData = [sunObj.alphas' svmObj.prob_values ]; %96 features
        cObj.trainLabels = obj.trainLabels;

        %Train meta svm classifier
        metaSvmObj = SVM;
        metaSvmObj.trainSvmMetaUsingExtendedSet(cObj,obj,svmObj,sunObj);
        acc = metaSvmObj.svmMetaTestOnProbWithExtendedSet96F(obj,sunObj,cObj);
