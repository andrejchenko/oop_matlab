function stacking_svm_Normed_Distances_Values_Sunsal_Sum1_5Pix_0NN_pines
    avgAccVec = [];
    iter = 100;
    for a = 1:iter
        ...
        cObj = CombineClass;
        cObj.combineMethods_SVM_Distances_SunalSum1_NoValidation(obj,nObj,svmObj,sunObj);

        cObj.trainData = [sunObj.alphas' svmObj.dist_values ]; % (80 sum to 1 normalized abundances + 120 zscore normalized distances) = 200 feature vector
        ...
        acc = metaSvmObj.svmMetaTestOnNormedDistancesWithExtendedSetSum(obj,sunObj,cObj);
        avgAccVec = [avgAccVec; acc];
    end
    avgAcc = mean(avgAccVec);
    save avgAcc_svmNormedDistances avgAcc
    save avgAccVec_svmNormedDistances avgAccVec
end
