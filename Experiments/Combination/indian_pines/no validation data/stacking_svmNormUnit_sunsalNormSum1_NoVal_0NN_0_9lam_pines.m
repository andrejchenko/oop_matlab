function stacking_svmNormUnit_sunsalNormSum1_NoVal_0NN_0_9lam_pines
    avgUnmixing = 0;
    iter = 100;
    for a = 1:iter
        ...
        cObj = CombineClass;
        cObj.combineMethodsSVMUnitNormedSunalSum1NoValidation(obj,nObj,svmObj,sunObj);
        ...
        acc = metaSvmObj.svmMetaTest_ProbUnitNormed_AbunSum1Sum(obj,sunObj,cObj);
        avgUnmixing = avgUnmixing + acc;
        fileName = fullfile('Experiments','Combination','indian_pines','results','text','stacking_svmNormUnit_sunsalNormSum1Sum_NoVal_0NN_0_9lam.txt');
    ...
    fileName = fullfile('Experiments','Combination','indian_pines','results','text','stacking_svmNormUnit_sunsalNormSum1Sum_NoVal_0NN_0_9lam.txt');
    fileID = fopen(fileName,'a');
    fprintf(fileID,'Average combined accuracy: %4.3f: %4.3f\n',avgUnmixing);
    fclose(fileID);
    stop2 = 1;
end
